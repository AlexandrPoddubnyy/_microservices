...
3) Т.к старые app UI+POST похоже не работает с новыми версиями монго (это разбиралось в предыдущих ДЗ).
...
5) + Отдельный раннер запушен на отдельном докер-хосте
...
7) Некоторые задания CD не запускаются, видимо из-за опции kubernetes: active (указанны в DZ)
